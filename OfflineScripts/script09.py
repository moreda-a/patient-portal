...
'''
# Process each patient and include decoded notes from diagnostic reports
for patient in patient_data:
    # Initialize a summary with basic patient info
    summary = f"Patient Name: {patient['name']}\nBirth Date: {patient['birthDate']}\n"
    
    # Include conditions
    if patient['conditions']:
        summary += f"Conditions:\n- {', '.join([c['code'] for c in patient['conditions']])}\n"
    
    # Include decoded notes from diagnostic reports
    summary += "Doctor's Notes:\n"
    for report in patient['diagnosticReports']:
        summary += f"- Report Code: {report['code']}\n"
        summary += f"  Date: {report['date']}\n"
        summary += f"  Notes: {report['decoded_notes']}\n"
    
    # Print the detailed summary for the patient
    print(f"Detailed Information for {patient['name']}:\n", summary)
